...
class MetadataExtractor:
    ...
    def __init__(
        ...
        Args:
            ...
            required_fields: Fields that must be non-null after the first
                             extraction pass; if any is missing, extract()
                             retries once with a larger document window.
                             If omitted, the retry triggers only when ALL
    ...
    @staticmethod
    def _build_content_sample(text: str) -> str:
        """
        Build a compact, representative sample of the document for the LLM.

        Strategy: keep the first PREFIX_CHARS characters (title/authors/header
        info lives there), then append the document's markdown heading outline
        so the LLM sees the structure of the ENTIRE document — not just the
        first ~2 pages. The outline costs little (~OUTLINE_CHARS max) but
        carries section topics needed for semantic fields like research_focus.
        """
        if len(text) <= MetadataExtractor.PREFIX_CHARS:
            return text

        prefix = text[: MetadataExtractor.PREFIX_CHARS]
        headings = "\n".join(
        ...
        outline = headings[: MetadataExtractor.OUTLINE_CHARS]
        return f"{prefix}\n\n## Document Outline\n{outline}"

    def _needs_escalation(self, metadata: Dict[str, Any]) -> bool:
        """Decide whether the first extraction pass missed too much.

        With required_fields configured: retry if any required field is null.
        Without: retry only when every field came back null (total miss).
        """
        if self.required_fields:
            return any(metadata.get(f) is None for f in self.required_fields)
        return all(v is None for v in metadata.values())

    def extract(self, text: str, stored_values: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Extract metadata from document text.

        The LLM sees a compact sample: the document prefix plus a markdown
        heading outline of the whole document (see _build_content_sample).
        If the first pass misses required fields (or everything), one retry
        runs with a larger window of the raw text.
        ...
        # First pass: cheap, structure-aware sample
        sample = self._build_content_sample(text)
        metadata = chain.invoke({"content": sample}).model_dump()

        # Escalation: retry once with a larger raw window — but only if the
        # document actually has more content than the sample already showed.
        if self._needs_escalation(metadata) and len(text) > len(sample):
            ...
            # Merge: prefer retry values, keep first-pass values it lost
            metadata = {
    ...
    def from_yaml(cls, llm, yaml_path: str) -> "MetadataExtractor":
        ...
        The YAML file must contain a 'fields' list. Each field supports:
          ...
          - required: true to trigger a larger-window retry when the field
            comes back null on the first extraction pass (optional)
